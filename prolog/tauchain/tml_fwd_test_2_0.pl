...
test(2) :-
  ain_test([(a(X),~b(Y)/(Y>X) ==> biggest(a)),
...
test(4) :-
    ain_test([(foo(X), bar(Y)/{X=:=Y} ==> foobar(X)),
         (foobar(X), go ==> found(X)),
	 (found(X), {X>=100} ==> big(X)),
